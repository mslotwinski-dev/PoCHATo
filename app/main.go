...
func runDesktopApp() {
	...
	window.Resize(fyne.NewSize(1280, 740))
...
func (ui *desktopUI) showAuthScreen() {
	...
	ui.win.SetContent(container.NewMax(
...
func (ui *desktopUI) showMainScreen() {
	...
	ui.friendList = widget.NewList(
		...
		func(id widget.ListItemID, object fyne.CanvasObject) {
			...
			if id >= 0 && id < len(ui.friends) {
				if ui.friends[id].FriendUsername != "" {
					label.SetText(ui.friends[id].FriendUsername)
				} else {
	...
	ui.requestList = widget.NewList(
		...
		func(id widget.ListItemID, object fyne.CanvasObject) {
			...
			if id >= 0 && id < len(ui.requests) {
				username := ui.requests[id].SenderUsername
				if username != "" {
					label.SetText(fmt.Sprintf("From %s", username))
				} else {
	...
	ui.requestList.OnSelected = func(id widget.ListItemID) {
		...
		if id >= 0 && id < len(ui.requests) {
			username := ui.requests[id].SenderUsername
			if username != "" {
				ui.requestStatus.SetText(fmt.Sprintf("Selected request from %s", username))
			} else {
	...
	friendPanel := container.NewBorder(
		container.NewVBox(
			...
			container.NewBorder(nil, nil, nil, addFriendButton, ui.newFriendEntry),
		),
		nil,
		nil,
		nil,
...
func (ui *desktopUI) openFriend(friend Friend) {
	...
	if friendName == "" {
		friendName = ui.shortID(friend.FriendUserID)
	}
	ui.friendHeader.SetText(fmt.Sprintf("Conversation with %s", friendName))
	...
	ui.runAsync(func() {
		...
		if err == nil {
			ui.runOnMain(func() {
				ui.messages = history
				ui.messageHeader.SetText(fmt.Sprintf("Messages with %s", friendName))
				ui.messageList.Refresh()
			})
		}
